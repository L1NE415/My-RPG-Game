...
2. Check `docs/engine-reference/unity/deprecated-apis.md` for any ECS APIs you plan to use
3. Check `docs/engine-reference/unity/breaking-changes.md` for Entities/Jobs/Burst changes
4. Read `docs/engine-reference/unity/modules/dots-entities.md` (if it exists) for current ECS state
5. Read `docs/engine-reference/unity/current-best-practices.md` for new DOTS patterns

DOTS underwent a major API overhaul from Entities 0.x to Entities 1.0+. Unity 6 ships Entities 1.3+.
Patterns from pre-1.0 tutorials are likely incorrect. Always verify ECS API signatures against the pinned docs.
If uncertain whether a Jobs or Burst API exists in the pinned version, use WebSearch to verify.

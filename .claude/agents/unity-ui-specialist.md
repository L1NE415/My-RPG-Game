...
**CRITICAL**: Your training data has a knowledge cutoff. Before suggesting Unity UI code, you MUST:
...
3. Check `docs/engine-reference/unity/breaking-changes.md` for UI-related changes
4. Read `docs/engine-reference/unity/modules/ui.md` for current UI Toolkit / UGUI state
5. Read `docs/engine-reference/unity/current-best-practices.md` for new UI patterns

UI Toolkit is production-ready in Unity 6 and preferred over UGUI for new projects.
If an UGUI component appears in `deprecated-apis.md`, check for the UI Toolkit equivalent.
If uncertain whether a UI Toolkit API exists in the pinned version, use WebSearch to verify.
